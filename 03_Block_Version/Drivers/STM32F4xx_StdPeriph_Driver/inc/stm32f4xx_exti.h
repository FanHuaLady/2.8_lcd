...
//初始化和配置功能
void EXTI_Init(EXTI_InitTypeDef* EXTI_InitStruct);		//使用结构体初始化
void EXTI_StructInit(EXTI_InitTypeDef* EXTI_InitStruct);//把传入的结构体赋默认值
...
void EXTI_ClearFlag(uint32_t EXTI_Line);				//对标志位进行清除并触发中断
ITStatus EXTI_GetITStatus(uint32_t EXTI_Line);			//获取指定的标志位是否置1
